...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="production-servlets" as="xs:string+"
    select="(
        'ResourceServlet',
        'EXistServlet',
        'XQueryServlet',
  ...
  <!-- 
        ...
        adopted from
  https://stackoverflow.com/questions/51432291/copy-entire-doctype-tag-from-input-document-using-xslt-2-0
    -->
  <xsl:template match="/">
  ...
  <xsl:template
    match="webapp:servlet-mapping[not(webapp:servlet-name = 'XQueryURLRewrite' or $env = 'development')]" />

  <!-- for the restxq-only experience we hijack the XQueryURLRewrite servlet-mapping (which defaults
  to '/*') -->
